...
### 📦 近期修复（PDF / 批处理 / 下载稳定性）
- 修复 **LLM 图片工具结束语早于图片发送** 的问题，现在会等待图片真正发送完成后再返回工具成功结果
- 修复 **PDF 只能在图片真正生成成功后** 才允许调用，避免图片还没出来就误触发 PDF
- 修复 **刚生成完图片时 PDF 收集不到图** 的问题，新增会话级图片缓存，PDF 会优先读取最近成功生成的图片
- 修复 **AstrBot File 组件参数错误** 导致 PDF 无法发送的问题，现已正确传入 `name` 和 `file`
- 修复 **批量处理误把历史上下文图片混进来** 的问题：
  - 当前消息里如果已经带图 / PDF，则只处理当前消息中的图片
...
### 📌 当前批处理与 PDF 行为说明
- 用户明确发送“这两张图 / 这几张图”时，如果当前消息里已经附带图片，则插件默认只处理当前消息里的这些图片，不再额外混入历史上下文图片
- `打包PDF / 图片转PDF / 合成PDF` 与 `shoubanhua_pack_images_to_pdf` 现在要求：
  - 图片已明确生成成功，或当前消息 / 会话缓存中已存在有效图片
  - 否则会直接拒绝打包，避免空 PDF 或错误 PDF
- 批量处理并输出 PDF 时，最终发出的 PDF 文件名会带随机后缀，避免临时文件冲突
...
| `#lm删除 <预设名>` | `lmd`、`lm删`、`删除预设` | 删除指定自定义预设，并清理绑定的参考图 |
...
| 指令 | 别名 | 说明 |
| :--- | :--- | :--- |
| `#手办化切换源 <序号>` | - | 切换当前使用的API信息源（管理员） |
| `#手办化查看源` | - | 查看所有信息源和Key列表（管理员） |
...
| `pdf_wait_timeout` | PDF 收集图片时的最长等待秒数 | 30 |
| `pdf_wait_poll_interval` | PDF 收集图片时的轮询间隔秒数 | 2 |
| `pdf_wait_stable_rounds` | PDF 收集图片时认定“数量稳定”的轮数 | 2 |
| `pdf_session_image_cache` | 会话中最近成功生成图片的缓存数量 | 20 |
| `result_image_download_retries` | 结果图外链下载重试次数 | 3 |
| `result_image_download_timeout` | 结果图外链下载超时时间（秒） | 60 |
...
- **强大的管理功能 (管理员限定)**：
  ...
  - **预设与参考图管理**：通过指令维护预设提示词及其绑定参考图，便于统一风格。
...
| 配置项 | 说明 |
| --- | --- |
| `generic_sources` | 多源API配置（模板列表），每个源可独立配置URL、Key池和模型，跨源跨Key自动轮询 |
...
在 `generic_sources` 中添加信息源时，可根据服务商协议选择模板：

| 模板 | 适用协议 | 说明 |
| --- | --- | --- |
| `OpenAI 兼容源` | POST + JSON（`chat/completions`、`images/generations`、`images/edits`） | 适用于大多数中转 API，支持多 Key 轮询，可开启 `优先直连图片接口` / `强制使用图片接口` |
| `落心秋API` | GET + 查询参数 | 接口形如 `?prompt=...&url=...&size=...&quality=...&apikey=...`，需填写完整接口地址与 apikey 池，可配置 `图像比例`（如 `1:1`、`9:16`）和 `图像质量`（如 `high`） |
| `NanoBanana` | POST + JSON | 请求体 `{prompt, url:[...]}`，apikey 拼在接口地址上；url 为数组，支持多张图片 |
| `Grok 视频源` | 视频生成专用 | 用于 `#手办化视频` 指令，独立于图片源配置 |

> **注意**：`落心秋API` 与 `NanoBanana` 的图片编辑（图生图）只接受可访问的 http(s) 图片链接，无法上传本地图片或 base64 数据，因此改图时必须发送/引用可访问的图片链接（可多张），否则会提示改用其他信息源；纯文生图不受限制，无需图片链接。
