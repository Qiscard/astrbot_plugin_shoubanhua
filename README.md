...
| `落心秋API` | GET + 查询参数 | 接口形如 `?prompt=...&size=...&quality=...&apikey=...`，需填写完整接口地址与 apikey 池，可配置 `图像比例`（如 `1:1`、`9:16`）和 `图像质量`（如 `high`） |
| `Grok 视频源` | 视频生成专用 | 用于 `#手办化视频` 指令，独立于图片源配置 |

> **注意**：`落心秋API` 的图片编辑参数只接受可访问的图片链接，无法直接上传图片数据，因此该源**仅支持文生图**；选择它进行图生图、批量处理或人设拍照时会直接提示改用其他信息源。
